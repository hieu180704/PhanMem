...
<body>
  ...
  function query_loc_gia_nho_hon($gialoc)
  {
    $query = "SELECT tbl_sanpham.*, tbl_giamgia.phantram 
    FROM tbl_sanpham 
    JOIN tbl_giamgia  ON tbl_sanpham.magiamgia = tbl_giamgia.magiamgia 
    WHERE tbl_sanpham.gia * (1 - (tbl_giamgia.phantram / 100)) < '$gialoc';";
    return $query;
  }

  function query_loc_gia_lon_hon($gialoc)
  {
    $query = "SELECT tbl_sanpham.*, tbl_giamgia.phantram 
    FROM tbl_sanpham 
    JOIN tbl_giamgia  ON tbl_sanpham.magiamgia = tbl_giamgia.magiamgia 
  ...
  function query_loc_gia_and($gialoc_a, $gialoc_b)
  {
    $query = "SELECT tbl_sanpham.*, tbl_giamgia.phantram 
    FROM tbl_sanpham 
    JOIN tbl_giamgia  ON tbl_sanpham.magiamgia = tbl_giamgia.magiamgia 
    WHERE tbl_sanpham.gia * (1 - (tbl_giamgia.phantram / 100)) > '$gialoc_a' AND tbl_sanpham.gia * (1 - (tbl_giamgia.phantram / 100)) < '$gialoc_b';";
    return $query;
  }

  if ($gia === 1) {
    $query = query_loc_gia_nho_hon(10000000);
  } else if ($gia === 2) {
    $query = query_loc_gia_and(10000000, 20000000);
  } else if ($gia === 3) {
    $query = query_loc_gia_and(20000000, 30000000);
  } else if ($gia === 4) {
    $query = query_loc_gia_lon_hon(30000000);
  }

  // if ($gia === 1) {
  //   $query = "SELECT * FROM tbl_sanpham WHERE gia > 10000000";
  // } else if ($gia === 2) {
  //   $query = "SELECT * FROM tbl_sanpham WHERE gia > 10000000 AND gia < 20000000";
  // } else if ($gia === 3) {
  //   $query = "SELECT * FROM tbl_sanpham WHERE gia > 20000000 AND gia < 30000000";
  // } else if ($gia === 4) {
  //   $query = "SELECT * FROM tbl_sanpham WHERE gia > 30000000";
  // }

  //VÍ DỤ LỌC THEO GIÁ GỐC
